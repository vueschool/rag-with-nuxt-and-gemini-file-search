...
export async function createIndexJob(params: {
  ...
  await useStorage("data").setItem(getJobKey(job.id), job);
  return job;
}

export async function getIndexJob(jobId: string) {
  return await useStorage("data").getItem<RAGIndexJob>(getJobKey(jobId));
}
...
export async function markIndexJobSucceeded(jobId: string) {
  ...
  await useStorage("data").setItem(getJobKey(jobId), next);
}
...
export async function markIndexJobFailed(params: {
  ...
  await useStorage("data").setItem(getJobKey(params.jobId), next);
}
